...
function run_acorn(
    ...
    # Read all data
    ############################
    println("  [1/10] Setting up paths...")
    flush(stdout)

    # Use provided paths or fall back to project_path from utils.jl
    ...
    out_path = "$(run_directory)/outputs/$(save_name)"

    if !isdir(out_path)
        mkpath(out_path)
    end

    println("  [2/10] Reading input files...")
    flush(stdout)
    ...
    println("  [3/10] Processing bus and branch data...")
    flush(stdout)
    ...
    # Load adjustments
    #############################
    println("  [4/10] Adjusting load data...")
    flush(stdout)
    ...
    # Add generators
    #############################
    println("  [5/10] Adding generators...")
    flush(stdout)
    ...
    # Interface limits
    #########################
    println("  [6/10] Setting up interface limits...")
    flush(stdout)
    ...
    # Optimization
    ########################
    println("  [7/10] Building optimization model...")
    println("    n_gen = $(size(genprop, 1)), n_bus = $(size(busprop, 1)), n_branch = $(size(branchprop, 1)), nt = $(nt)")
    flush(stdout)
    ...
    println("  [8/10] Adding hydro constraints...")
    flush(stdout)
    ...
    println("  [9/10] Running optimization...")
    flush(stdout)
    ...
    if termination_status(model) == MOI.OPTIMAL
        println("  [10/10] Optimization successful! Saving results...")
        flush(stdout)
        ...
        flush(stdout)
        error("Optimization did not converge to optimal solution")
    end
