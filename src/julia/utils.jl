...
# Paths
##############################################
project_path = "/Users/ls2236/Projects/acorn-julia"
...
function subtract_small_hydro(load, small_hydro, sim_year)
    ...
    # Check if there are any common columns beyond bus_id
    if length(common_cols) <= 1
        println("    Warning: No matching time columns between load and small_hydro for year $(sim_year)")
        return load
    end
    ...
    for i in eachindex(small_hydro_bus_ids)
        ...
        if bus_idx !== nothing && ncol(load) > 1
...
function subtract_solar_dpv(load, solar_dpv, sim_year)
    ...
    # Check if there are any common columns beyond bus_id
    if length(common_cols) <= 1
        println("    Warning: No matching time columns between load and solar_dpv for year $(sim_year)")
        return load
    end
